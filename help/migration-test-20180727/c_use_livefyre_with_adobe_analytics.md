...
uuid: ad8e8758-cd9b-4983-b5a8-0c4fec88a2b9
index: y
internal: n
snippet: y
translate: y
...
1. Edit the report suite by clicking on the report suite to modify, then navigate to ** ` Edit Settings > Conversion > Success Events` **.
...
1. In the Report Suite Manager click ** ` Edit Settings > Conversion > Conversion Variables` **.
...
1. If an existing Adobe Analytics Tool does not exist, click the ** ` Add a Tool` ** button. 
   Set the following parameters for the tool:

    * Set ** ` Tool Type` ** to ** ` Adobe Analytics` **.
    * Enable ** ` Automatic Configuration` **
    * Enable ** ` Authenticate via Marketing Cloud` **

1. Add or confirm the name of the report suite with Livefyre events to the ** ` Report Suites` ** field.
...
1. In DTM, select ** ` Rules` ** tab.
1. Click ** ` Page Load Rules` **.
1. Click on the ** ` Create New Rule` ** button.
1. Open the ** ` Conditions` ** section by clicking on the ** ` Plus` ** button.
1. Trigger the rule. Choose ** ` DOM Ready` ** or ** ` Onload` ** trigger types if you want to delay or implement the rule asynchronously.
...
1. Click ** ` Approve` **.
1. Click ** ` Yes, approve` ** to confirm your approval.
1. Go to ** ` Overview > Publish Queue` **.
1. Select the Rule to publish.
1. Click ** ` Publish Selected` **.
1. Click ** ` Publish` ** to confirm that you want to publish.
