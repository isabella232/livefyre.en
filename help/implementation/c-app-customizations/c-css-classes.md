...
|  Class  | Description  |
|---|---|
|  .fyre-comment-author-tag- *`custom tag name`* | Livefyre will create a CSS class in this format for each user tag added through Livefyre’s Studio, [Profile Sync](t-about-identity-integration/t-sync-with-livefyre-using-ping-for-pull/t-sync-with-livefyre-using-ping-for-pull.md). This class may be used to style the background for any content posted by user accounts including that tag.  |
