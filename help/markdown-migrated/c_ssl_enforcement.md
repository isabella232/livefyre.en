...
1. Change hostnames from `codeph {*}.&lt;network&gt;.fyre.co` to `codeph &lt;network&gt;.{*}.fyre.co`. For example, the host name `codeph example.network.fyre.co` changes to `codeph network.example.fyre.co`.
...
Livefyre does not have SSL certificates on `codeph {*}.&lt;network&gt;.fyre.co` domains. Simply changing the URL to HTTPS breaks the application.
